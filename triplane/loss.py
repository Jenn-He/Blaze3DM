...
def eir_loss(obj_idx, auto_decoder, device='cuda', offset_distance=0.01):
    num_points = 10000
    random_coords = torch.rand(obj_idx.shape[0], num_points, 3).to(device) * 2 - 1  # sample from [-1, 1]
    offset_coords = random_coords + torch.randn_like(
        random_coords) * offset_distance  # Make offset_magnitude bigger if you want smoother
